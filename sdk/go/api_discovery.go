...
type ApiOauthProtectedResourceMcpRequest struct {
	ctx context.Context
	ApiService *DiscoveryAPIService
}

func (r ApiOauthProtectedResourceMcpRequest) Execute() (map[string]interface{}, *http.Response, error) {
	return r.ApiService.OauthProtectedResourceMcpExecute(r)
}

/*
OauthProtectedResourceMcp Protected-resource metadata for the MCP endpoint (RFC 9728)
...
Path-inserted protected-resource metadata used by MCP OAuth clients. Served by the MCP sidecar when one is configured, because it is the authority on the scope bundle its own tools require; otherwise it falls back to the resource-root document, since `/v0` and `/mcp` share AgentDrive's canonical audience.
 ...
 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiOauthProtectedResourceMcpRequest
*/
func (a *DiscoveryAPIService) OauthProtectedResourceMcp(ctx context.Context) ApiOauthProtectedResourceMcpRequest {
	return ApiOauthProtectedResourceMcpRequest{
		ApiService: a,
		ctx: ctx,
	}
...
// Execute executes the request
//  @return map[string]interface{}
func (a *DiscoveryAPIService) OauthProtectedResourceMcpExecute(r ApiOauthProtectedResourceMcpRequest) (map[string]interface{}, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "DiscoveryAPIService.OauthProtectedResourceMcp")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}
