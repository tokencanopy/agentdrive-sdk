...
class WriteCellsIn(BaseModel):
    """
    Batched by design: chatty per-range writes to one workbook are the main source of throttling and unrecoverable partial state (Microsoft's documented Excel-API lesson), so one request carries the whole batch and it applies atomically or not at all.
    """ # noqa: E501
    writes: Annotated[List[CellWriteIn], Field(min_length=1)]
    __properties: ClassVar[List[str]] = ["writes"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of WriteCellsIn from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of WriteCellsIn from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "writes": [CellWriteIn.from_dict(_item) for _item in obj["writes"]] if obj.get("writes") is not None else None
        })
        return _obj
